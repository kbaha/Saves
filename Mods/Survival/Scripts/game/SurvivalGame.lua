...
function BindDevCommands()
	...
	-- Gamemode
	sm.game.bindChatCommand( "/survival", {}, "cl_onChatCommand", "Change game-mode to survival")
	sm.game.bindChatCommand( "/s", {}, "cl_onChatCommand", "Short-cut to '/survival'")
	sm.game.bindChatCommand( "/creative", {}, "cl_onChatCommand", "Change game-mode to creative")
	sm.game.bindChatCommand( "/c", {}, "cl_onChatCommand", "Short-cut to '/creative'")
	
	--Give
	sm.game.bindChatCommand( "/gas", { { "int", "quantity", true } }, "cl_onChatCommand", "Give <quantity> gasoline (default 10)" )
	sm.game.bindChatCommand( "/battery", { { "int", "quantity", true } }, "cl_onChatCommand", "Give <quantity> batteries (default 10)" )
...
function SurvivalGame.client_onCreate( self )
	...
	sm.game.bindChatCommand( "/cheat", {}, "cl_onChatCommand", "Enables Cheats")
...
function SurvivalGame.cl_onChatCommand( self, params )
	...
	elseif params[1] == "/cheat" then
		g_survivalDev = true
		BindDevCommands()
	elseif params[1] == "/creative" or params[1] == "/c" then
		self.network:sendToServer( "sv_setGodMode", true )
		self.network:sendToServer( "sv_setLimitedInventory", false )
	elseif params[1] == "/survival" or params[1] == "/s" then
		self.network:sendToServer( "sv_setGodMode", false )
		self.network:sendToServer( "sv_setLimitedInventory", true )
	elseif params[1] == "/gas" then
		self.network:sendToServer( "sv_giveItem", { player = sm.localPlayer.getPlayer(), item = obj_consumable_gas, quantity = ( params[2] or 10 ) } )
	elseif params[1] == "/battery" then
		self.network:sendToServer( "sv_giveItem", { player = sm.localPlayer.getPlayer(), item = obj_consumable_battery, quantity = ( params[2] or 10 ) } )
	else
...
function SurvivalGame.sv_setGodMode( self, state )
	g_godMode = state
	self.network:sendToClients( "client_showMessage", "GODMODE: " .. ( g_godMode and "On" or "Off" ) )
end
